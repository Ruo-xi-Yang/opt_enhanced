...
batch_size = 8                  # how many populations are evaluated at the same time
...
class MyProblem(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):        
        ...
        if (ngen == evaluated_gen and initialseeding == "evalPOP"):
            ...
            #pre-process ------modified

            # i = 0
            # for index, row in enumerate(sorted_population):
    ...
    def generate_essential_files(self,path,ngen,i,dv1,dv2):
        ...
        # job file
        if optimisation == "3D":
            trialfile.cirrus_jobfile(ngen,path,GPUs,wctimep,i,dv1,dv2,AoA,pert_ini_file,
                               main_ini_file,pert_sol_file,"p",optimisation)
            
        trialfile.cirrus_jobfile(ngen,path,GPUs,wctime,i,dv1,dv2,AoA,pert_ini_file,main_ini_file,
                             pert_sol_file,"u",optimisation)
        # main ini file
        trialfile.ini_file(path,main_ini_file,gamma,mu,Pr,order,tstart,tend,dt)
        trialfile.plugin_airfoilforces(path,main_ini_file)
        trialfile.plugin_soln_writer(path,main_ini_file,dt_out,ngen,i)
        trialfile.plugin_soln_avg(path,optimisation,main_ini_file,avg_from,tend,ngen,i)
        trialfile.plugin_sampler(path,optimisation,main_ini_file)
        trialfile.boundary_conditions(path,optimisation,main_ini_file,0,u_inf,rho_inf,p_inf)
        # second ini file
        trialfile.ini_file(path,second_ini_file,gamma,mu,Pr,order,tstart,tend,dt)
        trialfile.plugin_airfoilforces_second(path,second_ini_file)
        trialfile.plugin_soln_writer(path,second_ini_file,dt_out,ngen,i)
        trialfile.plugin_soln_avg(path,optimisation,second_ini_file,avg_from,tend,ngen,i)
        trialfile.plugin_sampler(path,optimisation,second_ini_file)
        trialfile.boundary_conditions(path,optimisation,second_ini_file,0,u_inf_2,rho_inf,p_inf)
        # first perturbation ini file
        trialfile.ini_file(path,pert_ini_file,gamma,mu,Pr,order,tstart,tperturb,dt)
        trialfile.plugin_airfoilforces(path,pert_ini_file)
        trialfile.plugin_soln_writer(path,pert_ini_file,tperturb,ngen,i)
        trialfile.plugin_sampler(path,optimisation,pert_ini_file)
        trialfile.boundary_conditions(path,optimisation,pert_ini_file,1,u_inf,rho_inf,p_inf)
        #second perturbation file 
        trialfile.ini_file(path,sec_pert_ini_file,gamma,mu,Pr,order,tstart,tperturb,dt)
        trialfile.plugin_airfoilforces(path,sec_pert_ini_file)
        trialfile.plugin_soln_writer(path,sec_pert_ini_file,tperturb,ngen,i)
        trialfile.plugin_sampler(path,optimisation,sec_pert_ini_file)
        trialfile.boundary_conditions(path,optimisation,sec_pert_ini_file,1,u_inf_2,rho_inf,p_inf)
        # gmsh geo file
        trialfile.gmsh_file(ngen,i,path,dv1,dv2,AoA,optimisation)
        # eval.sh file
        trialfile.eval_file(ngen,i,path,optimisation,AoA,GPUs)
        trialfile.eval_file_second(ngen,i,path,optimisation,AoA,GPUs)
